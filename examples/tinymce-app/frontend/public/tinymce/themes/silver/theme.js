...
(function () {
  ...
  const is$2 = (value, constructor) =>
    isObject(value) && hasProto(value, constructor, (o, proto) => getPrototypeOf$1(o) === proto);
  ...
  const cleanParameters = (parameters) =>
    map$2(parameters, (p) => (endsWith(p, '/*') ? p.substring(0, p.length - '/*'.length) : p));
  ...
  const combineGroups = (byEventName, eventOrder) => {
    const r = mapToArray(byEventName, (tuples, eventName) => {
      ...
      return combined.map((handler) => {
        ...
        const purpose =
          tuples.length > 1
            ? filter$2(eventOrder[eventName], (o) => exists(tuples, (t) => t.name === o)).join(' > ')
            : tuples[0].name;
  ...
  const renderToDom = (definition, optObsoleted) => {
    const canBePatched = (candidate) =>
      name$3(candidate) === definition.tag && !hasMixedChildren(definition) && !isPremade(candidate);
    ...
    const status = get$g(eventConfig.get(), eventName)
      .orThunk(() => {
        ...
        return findMap(patterns, (p) =>
          eventName.indexOf(p) > -1 ? Optional.some(eventConfig.get()[p]) : Optional.none()
        );
      })
  ...
  const getTrace = () => {
    ...
    if (err.stack !== undefined) {
      ...
      return find$5(lines, (line) => line.indexOf('alloy') > 0 && !exists(path, (p) => line.indexOf(p) > -1)).getOr(
        unknown
      );
    } else {
      return unknown;
  ...
  const tieredMenuMarkers = () =>
    requiredObjOf('markers', [required$1('backgroundMenu')].concat(menuFields()).concat(itemFields()));
  ...
  const onKeyboardHandler = (fieldName) =>
    onPresenceHandler('onKeyboardHandler', fieldName, defaulted$1(Optional.none));
  ...
  const chooseChannels = (channels, message) =>
    message.universal ? channels : filter$2(channels, (ch) => contains$2(message.channels, ch));
  ...
  const attempts = (element, candidates, anchorBox, elementBox, bubbles, bounds) => {
    ...
    const attemptBestFit = (layout, reposition, visibleW, visibleH, isVisible) => {
      ...
      return attemptLayout.fold(constant$1(attemptLayout), (newReposition, newVisibleW, newVisibleH, newIsVisible) => {
        const improved =
          isVisible === newIsVisible ? newVisibleH > visibleH || newVisibleW > visibleW : !isVisible && newIsVisible;
  ...
  const isBottomToTopDir = (el) =>
    closest$2(
      el,
      (current) => isElement$1(current) && get$f(current, 'data-alloy-vertical-dir') === AttributeValue.BottomToTop
    );
  ...
  const dataset = () => {
    ...
    const lookup = (itemString) =>
      get$g(dataByValue.get(), itemString).orThunk(() => get$g(dataByText.get(), itemString));
    const update = (items) => {
  ...
  const field = (name, forbidden) =>
    defaultedObjOf(
      name,
      {},
      map$2(forbidden, (f) => forbid(f.name(), 'Cannot configure ' + f.name() + ' for ' + name)).concat([
  ...
  const hasNative = (component, config) =>
    config.useNative === true && contains$2(nativeDisabled, name$3(component.element));
  ...
  const exhibit$5 = (base, disableConfig) =>
    nu$7({classes: disableConfig.disabled() ? disableConfig.disableClass.toArray() : []});
  ...
  const horizontal = (container, selector, current, delta) => {
    const isDisabledButton = (candidate) =>
      name$3(candidate) === 'button' && get$f(candidate, 'disabled') === 'disabled';
  ...
  const findCurrent = (component, flowConfig) =>
    flowConfig.focusManager.get(component).bind((elem) => closest$1(elem, flowConfig.selector));
  ...
  const make$6 = (detail, _rawUiSpec) => {
    ...
    const toDirectory = (_container) =>
      map$1(detail.data.menus, (data, _menuName) =>
        bind$3(data.items, (item) => (item.type === 'separator' ? [] : [item.data.value]))
      );
    ...
    const getMenus = (state, menuValues) =>
      cat(
        map$2(menuValues, (mv) =>
          state.lookupMenu(mv).bind((prep) => (prep.type === 'prepared' ? Optional.some(prep.menu) : Optional.none()))
        )
      );
  ...
  const factory$o = (detail, spec) => {
    ...
    const showWithinBounds = (sandbox, thing, placementSpec, getBounds) => {
      ...
      Sandboxing.openWhileCloaked(sandbox, thing, () =>
        Positioning.positionWithinBounds(sink, sandbox, placementSpec, getBounds())
      );
  ...
  const contextToolbarSchema = objOf(
    [defaultedType('contexttoolbar'), requiredString('items')].concat(contextBarFields)
  );
  ...
  const cardMenuItemSchema = objOf(
    [type, optionalLabel, requiredArrayOf('items', itemSchema$1), onSetup, defaultedOnAction].concat(
  ...
  const getCurrentColor = (editor, format) => {
    const cssRgbValue = get$e(
      SugarElement.fromDom(editor.selection.getStart()),
      format === 'hilitecolor' ? 'background-color' : 'color'
    );
  ...
  const renderFancyMenuItem = (spec, backstage) =>
    get$g(fancyMenuItems, spec.fancytype).map((render) => render(spec, backstage));
    ...
    const apis = {
      ...
      refetch: (comp) => {
        ...
        return optSandbox.fold(
          ...
          (sandboxComp) => {
            return open(detail, identity, comp, sandboxComp, externals, noop, HighlightOnOpen.HighlightMenuAndItem).map(
              noop
            );
          }
        );
  ...
  const handleRefetchTrigger = (originalSandboxComp) => {
    ...
    Dropdown.refetch(dropdown).get(() => {
      ...
      optSearcherState.each((searcherState) =>
        findWithinSandbox(newSandboxComp).each((inputComp) => restoreState(inputComp, searcherState))
      );
    });
  };
    ...
    switch (item.type) {
      ...
      case 'nestedmenuitem':
        return createNestedMenuItem(item).fold(handleError, (d) =>
          Optional.some(
            nested(parseForHorizontalMenu(d), itemResponse, providersBackstage, menuHasIcons, isHorizontalMenu)
          )
        );
      case 'togglemenuitem':
  ...
  const createPartialMenu = (value, items, itemResponse, backstage, isHorizontalMenu, searchMode) => {
    ...
    const alloyItems = cat(
      map$2(items, (item) => {
        ...
        const createItem = (i) =>
          createMenuItemFromBridge(i, itemResponse, backstage, itemHasIcon(i), isHorizontalMenu);
    ...
    const autocompleter = build$1(
      InlineView.sketch({
        ...
        inlineBehaviours: derive$1([
          config('dismissAutocompleter', [run$1(dismissRequested(), () => cancelIfNecessary())])
        ]),
        lazySink: sharedBackstage.getSink
      })
    );
  ...
  const takeover = (root) => {
    ...
    const getByUid = (uid) =>
      registry
        .getById(uid)
        .fold(
          () => Result.error(new Error('Could not find component with uid: "' + uid + '" in system.')),
          Result.value
        );
    const getByDom = (elem) => {
  ...
  const rgbFormFactory = (translate, getClass, onValidHexx, onInvalidHexx) => {
    ...
    const factory = () => {
      ...
      const getValueRgb = () =>
        get('red').bind((red) =>
          get('green').bind((green) => get('blue').map((blue) => rgbaColour(red, green, blue, 1)))
        );
      ...
      return deepMerge(
        Form.sketch((parts) => ({
          ...
          components: [
            parts.field(
              'red',
              FormField.sketch(renderTextField(isRgbaComponent, 'red', redStrings.label, redStrings.description, 255))
            ),
            parts.field(
              'green',
              FormField.sketch(
                renderTextField(isRgbaComponent, 'green', greenStrings.label, greenStrings.description, 255)
              )
            ),
            parts.field(
              'blue',
              FormField.sketch(
                renderTextField(isRgbaComponent, 'blue', blueStrings.label, blueStrings.description, 255)
              )
            ),
            parts.field(
              'hex',
              FormField.sketch(renderTextField(isHexString, 'hex', hexStrings.label, hexStrings.description, 'ffffff'))
            ),
            memPreview.asSpec()
          ],
  ...
  const filterByExtension = (files, providersBackstage) => {
    ...
    const isFileInAllowedTypes = (file) =>
      exists(allowedImageFileTypes, (type) => endsWith(file.name.toLowerCase(), `.${type.toLowerCase()}`));
  ...
  const renderCommonDropdown = (spec, prefix, sharedBackstage) => {
    ...
    const optMemDisplayIcon = spec.icon.map((iconName) =>
      record(renderReplaceableIconFromPack(iconName, sharedBackstage.providers.icons))
    );
  ...
  const renderSizeInput = (spec, providersBackstage) => {
    ...
    const heightField = FormCoupledInputs.parts.field2(
      formGroup([FormField.parts.label(getLabel('Height')), getFieldPart(false)])
    );
  ...
  const make$3 = (detail, components, spec, externals) => {
    const navigateList = (comp, simulatedEvent, highlighter) => {
      ...
      if (Sandboxing.isOpen(sandbox)) {
        ...
        open(detail, mapFetch(comp), comp, sandbox, externals, onOpenSync, HighlightOnOpen.HighlightMenuAndItem).get(
          noop
        );
      }
    };
      ...
      config(
        ...
        [
          ...
          runOnExecute$1((comp) => {
            ...
            togglePopup(detail, mapFetch(comp), comp, externals, onOpenSync, HighlightOnOpen.HighlightMenuAndItem).get(
              noop
            );
          }),
      ...
      (linkInfo) => {
        ...
        return fileType === 'file'
          ? joinMenuLists([
              ...
              filterByQuery(
                term,
                flatten([anchorTargetTop(linkInfo), anchorTargets(linkInfo), anchorTargetBottom(linkInfo)])
              )
            ])
          : history;
      }
  ...
  const getPickerTypes = (editor) => {
    ...
    return getPicker(editor).fold(never, (_picker) =>
      optFileTypes.fold(always, (types) => (keys(types).length > 0 ? types : false))
    );
  };
  ...
  const factory$f = (detail, components, _spec, _external) => {
    ...
    const getListContainer = (component) =>
      detail.shell ? Optional.some(component) : getPart(component, detail, 'items');
    return {
      uid: detail.uid,
      dom: detail.dom,
  ...
  const factory$e = (detail, components, _spec, _externals) => {
    ...
    const getGroupContainer = (component) =>
      detail.shell ? Optional.some(component) : getPart(component, detail, 'groups');
    const extra = detail.shell
      ? {
          behaviours: [Replacing.config({})],
          components: []
  ...
  const getPrior = (elem, state) =>
    state.getInitialPos().map((pos) => bounds(pos.bounds.x, pos.bounds.y, get$c(elem), get$d(elem)));
  ...
  const groupToolbarButtonSchema = objOf(
    [type, requiredOf('items', oneOf([arrOfObj([name$1, requiredArrayOf('items', string)]), string]))].concat(
  ...
  const findFocusedComp = (comps) =>
    findMap(comps, (comp) =>
      search(comp.element).bind((focusedElm) => comp.getSystem().getByDom(focusedElm).toOptional())
    );
  ...
  const renderSplitButton = (spec, sharedBackstage) => {
    ...
    const getApi = (comp) => ({
      ...
      isActive: () =>
        descendant(comp.element, 'span').exists((button) => comp.getSystem().getByDom(button).exists(Toggling.isOn))
    });
    const editorOffCell = Cell(noop);
  ...
  const getAnchorSpec$2 = (editor, mobile, data, position) => {
    ...
    const desktopAnchorSpecLayouts = {
      onLtr: (elem) =>
        [north$2, south$2, northeast$2, southeast$2, northwest$2, southwest$2].concat(getInsetLayouts(elem)),
      onRtl: (elem) =>
        [north$2, south$2, northwest$2, southwest$2, northeast$2, southeast$2].concat(getInsetLayouts(elem))
    };
    const mobileAnchorSpecLayouts = {
      onLtr: (elem) =>
        [south$2, southeast$2, southwest$2, northeast$2, northwest$2, north$2].concat(getInsetLayouts(elem)),
      onRtl: (elem) =>
        [south$2, southwest$2, southeast$2, northwest$2, northeast$2, north$2].concat(getInsetLayouts(elem))
    };
  ...
  const lookup$1 = (scopes, editor) => {
    ...
    return matchStartNode(startNode, scopes.inNodeScope, scopes.inEditorScope).orThunk(() =>
      matchAncestor(isRoot, startNode, scopes)
    );
  };
  ...
  const register$9 = (editor, registryContextToolbars, sink, extras) => {
    ...
    const buildToolbar = (toolbars) => {
      ...
      const toolbarType =
        getToolbarMode(editor) === ToolbarMode$1.scrolling ? ToolbarMode$1.scrolling : ToolbarMode$1.default;
  ...
  const getSelectedElement = (editor, e) =>
    isTriggeredByKeyboard(editor, e) ? editor.selection.getStart(true) : e.target;
  ...
  const asAbsolute = (coord, scroll, origin) =>
    coord.fold(transform([add(origin)]), transform([]), transform([add(scroll)]));
    ...
    const renderMainUi = () => {
      ...
      const statusbar =
        useStatusBar(editor) && !isInline
          ? Optional.some(renderStatusbar(editor, backstages.popup.shared.providers))
          : Optional.none();
    ...
    return [
      ...
      fireApiEvent(formActionEvent, (api, spec, event, component) => {
        ...
        const isDisabled = (focused) =>
          has$1(focused, 'disabled') || getOpt(focused, 'aria-disabled').exists((val) => val === 'true');
  ...
  const lookup = (compInSystem, footerButtons, buttonName) =>
    find$5(footerButtons, (button) => button.name === buttonName).bind((memButton) =>
      memButton.memento.getOpt(compInSystem)
    );
    ...
    const dialog = build$1({
      ...
      behaviours: derive$1([
        Keying.config({
          ...
          useTabstopAt: (elem) =>
            !isPseudoStop(elem) && (name$3(elem) !== 'button' || get$f(elem, 'disabled') !== 'disabled')
        }),
        Reflecting.config({
  ...
  const isAlertOrConfirmDialog = (target) =>
    closest(target, '.tox-alert-dialog') || closest(target, '.tox-confirm-dialog');
  ...
  var Theme = () => {
    global$a.add('silver', (editor) => {
      ...
      const getNotificationManagerImpl = () =>
        NotificationManagerImpl(editor, {backstage: popups.backstage}, popups.getMothership());
      return {
        renderUI,
